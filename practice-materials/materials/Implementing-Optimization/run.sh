...
if [ "$#" -ne 2 ]; then
  ...
  echo "ex)  ./run.sh all ~/opt/llvm-22.1.0/bin"
  exit 1
fi
...
if [[ "$1" != "run" && "$1" != "test" ]]; then
  ...
  CXX=$2/clang++
  CXXFLAGS="$CXXFLAGS $PICFLAG -g"
  set -e

  $CXX $CXXFLAGS $ISYSROOT $LDFLAGS $LIBS constfold.cpp -o ./libConstFold$EXT -shared
  $CXX $CXXFLAGS $ISYSROOT $LDFLAGS $LIBS fillundef.cpp -o ./libFillUndef$EXT -shared
  $CXX $CXXFLAGS $ISYSROOT $LDFLAGS $LIBS instmatch.cpp -o ./libInstMatch$EXT -shared
fi
