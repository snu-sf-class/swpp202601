...
if [ "$#" -ne 2 ]; then
  ...
  echo "ex) ./run-passes.sh all ~/opt/llvm-22.1.0/bin"
  exit 1
fi
...
if [ "$1" != "run" ]; then
  ...
  $CXX $CXXFLAGS $ISYSROOT $LDFLAGS $LIBS hello.cpp -o ./libHello$EXT -shared
  $CXX $CXXFLAGS $ISYSROOT $LDFLAGS $LIBS dump.cpp -o ./libDump$EXT -shared
