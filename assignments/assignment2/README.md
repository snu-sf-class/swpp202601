...
### 0. Set up workspace
Open assignment 2 directory in vscode.
Edit and move `example-settings.json` into `.vscode` directory.
vscode CMake extension will pick this up and configure the workspace
with proper toolchains.
...
If your implementation passes every test, you've correctly implemented it.  
However, if your `move_collect_expected` is slower than `collect_expected`,
try implementing the move operation more carefully.
